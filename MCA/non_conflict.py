...
ds_distance_to_morocco.loc[ds_distance_to_morocco == 0] = 100

# Rang-skale
kriterium = (ds_ratio_distance > 0) & (ds_ratio_distance < 100) | (ds_ratio_distance > 100)

ds_ranks = ds_ratio_distance.loc[kriterium].rank(ascending=False).astype(int)-1
...
gdf_grid.plot(column='non_conflict', cmap='YlGnBu', legend=True)